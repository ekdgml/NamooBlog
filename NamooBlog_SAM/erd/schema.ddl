...
ALTER TABLE author_tb
	ADD CONSTRAINT PK_author_tb -- 작성자 Primary key
		PRIMARY KEY (
			author_id -- 작성자ID
		);
...
-- 블로그
ALTER TABLE blog_tb
	ADD CONSTRAINT PK_blog_tb -- 블로그 Primary key
		PRIMARY KEY (
			blog_id -- 블로그ID
		);
...
-- 포스트
ALTER TABLE post_tb
	ADD CONSTRAINT PK_post_tb -- 포스트 Primary key
		PRIMARY KEY (
			post_id -- 포스트ID
		);
...
ALTER TABLE comment_tb
	ADD CONSTRAINT PK_comment_tb -- 코멘트 Primary key
		PRIMARY KEY (
			comment_id -- 코멘트ID
		);
